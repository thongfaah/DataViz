...
//         <XAxis dataKey={selectedFile && selectedColumns[selectedFile]?.length > 0 ? selectedColumns[selectedFile][0] : ''} />
//         <YAxis />
//         <Tooltip />
//         <Legend />
//         {selectedFile && selectedColumns[selectedFile]?.slice(1).map((col, index) => (
//           <Bar key={index} dataKey={col} fill={`#${Math.floor(Math.random() * 16777215).toString(16)}`} />
//         ))}
//       </BarChart>
//     </ResponsiveContainer>
...
//     <div className={`fixed top-[9.7rem] h-full right-0 h-full w-64 bg-white shadow-md border-l border-gray-300 transition-transform ${isSidebarOpen ? "translate-x-0" : "translate-x-full"}`}>
//       <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="absolute top-4 -left-10 bg-gray-200 p-2 rounded-l-md">
//         {isSidebarOpen ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
//       </button>
